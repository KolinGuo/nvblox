"""
Setup script for nvblox_torch.

This script handles version generation and is compatible with scikit-build-core.
For in-place builds, use: pip install -e .
"""
import os
import subprocess
...
def get_version_with_platform_tag() -> str:
    ...
    if cuda_version and ubuntu_version:
        return f'{NVBLOX_VERSION}+cu{cuda_version}ubuntu{ubuntu_version}'
    return NVBLOX_VERSION
...
def create_version_file() -> None:
    ...
    version_path = os.path.join(os.path.dirname(__file__), 'nvblox_torch', '_version.py')
...
# Always generate the version file when this module is imported or run
create_version_file()


def get_version() -> str:
    """Return the version string for scikit-build-core."""
    return get_version_with_platform_tag()


# This function is called by scikit-build-core to get the version dynamically
def dynamic_metadata():
    """Return dynamic metadata for scikit-build-core."""
    return {"version": get_version()}


if __name__ == '__main__':
    # When run directly, just print the version
    print(f"nvblox_torch version: {get_version_with_platform_tag()}")
